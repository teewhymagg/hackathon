## hackathon_bot Bot — Platform‑agnostic Meeting Bot
...
await runMeetingFlow("google_meet", botConfig, page, gracefulLeave, {
  join: async (page, cfg) =>
    joinGoogleMeeting(page, cfg.meetingUrl!, cfg.botName, cfg),
...
```bash
cd hackathon/services/hackathon-bot
...
- Single hot-bot identity: channel `bot_commands:hot-debug`, container `hackathon_bot-hot`
- No local `node_modules` needed (uses Docker)

Build only:

```bash
cd hackathon/services/hackathon-bot/core
npm run build
```
...
```bash
docker build -t hackathon_bot .
```

This command will create a Docker image named hackathon_bot.
...
```bash
docker run -e BOT_CONFIG='{"platform": "google_meet", "meetingUrl": "https://meet.google.com/xcb-tssj-qjc", "botName": "hackathon_bot", "token": "123", "connectionId": "", "automaticLeave": {"waitingRoomTimeout": 300000, "noOneJoinedTimeout": 300000, "everyoneLeftTimeout": 300000}}' hackathon_bot
```
