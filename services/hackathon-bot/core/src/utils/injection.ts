...
export async function ensureBrowserUtils(page: Page, scriptFsPath: string): Promise<void> {
  ...
  const already = await page.evaluate(() => !!(window as any).hackathon_botBrowserUtils);
  if (already) return;

  // 1) Try simple addScriptTag (works when bypassCSP true)
  try {
    await page.addScriptTag({ path: scriptFsPath });
    const ok = await page.evaluate(() => !!(window as any).hackathon_botBrowserUtils);
    if (ok) return;
  } catch (e: any) {
  ...
  try {
    ...
    await page.evaluate(async (script) => {
      const injectWithTrustedTypes = () => {
        ...
        const policy = factory.createPolicy('hackathon_botPolicy', {
  ...
  // 3) Verify
  const loaded = await page.evaluate(() => !!(window as any).hackathon_botBrowserUtils);
  if (!loaded) {
    throw new Error('hackathon_botBrowserUtils global is missing after injection');
  }
}
