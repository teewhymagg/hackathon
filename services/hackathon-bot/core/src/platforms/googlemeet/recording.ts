...
export async function startGoogleRecording(page: Page, botConfig: BotConfig): Promise<void> {
  ...
  await page.evaluate(
    async (pageArgs: {
      ...
      const browserUtils = (window as any).hackathon_botBrowserUtils;
      ...
      (window as any).triggerWebSocketReconfigure = async (lang: string | null, task: string | null) => {
        try {
          ...
          await svc?.connectToWhisperLive(
            cfg,
            (window as any).__hackathon_botOnMessage,
            (window as any).__hackathon_botOnError,
            (window as any).__hackathon_botOnClose
          );
            ...
            const connectWhisper = async () => {
              try {
                ...
                const onMessage = (data: any) => {
                  ...
                  if (data["language"]) {
                    if (!(window as any).__hackathon_botLangLogged) {
                      (window as any).__hackathon_botLangLogged = true;
              ...
              function getGoogleParticipantId(element: HTMLElement) {
                ...
                if (!id) {
                  if (!(element as any).dataset.hackathon_botGeneratedId) {
                    (element as any).dataset.hackathon_botGeneratedId = 'gm-id-' + Math.random().toString(36).substr(2, 9);
                  }
              ...
              function scanForAllGoogleParticipants() {
                ...
                for (const sel of participantSelectors) {
                  document.querySelectorAll(sel).forEach((el) => {
                    ...
                    if (!(elh as any).dataset.hackathon_botObserverAttached) {
