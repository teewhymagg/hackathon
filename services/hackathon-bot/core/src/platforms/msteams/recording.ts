...
export async function startTeamsRecording(page: Page, botConfig: BotConfig): Promise<void> {
  ...
  await page.evaluate(
    async (pageArgs: {
      ...
      const { BrowserAudioService, BrowserWhisperLiveService } = (window as any).hackathon_botBrowserUtils;

      // --- Early reconfigure wiring (event listener only) ---
      (window as any).__hackathon_botPendingReconfigure = null;
      try {
        document.addEventListener('hackathon_bot:reconfigure', (ev: Event) => {
          try {
      ...
      (window as any).__hackathon_botWhisperLiveService = whisperLiveService;
      (window as any).__hackathon_botBotConfig = botConfigData;
      ...
      (window as any).triggerWebSocketReconfigure = async (lang: string | null, task: string | null) => {
        try {
          const svc = (window as any).__hackathon_botWhisperLiveService;
          const cfg = (window as any).__hackathon_botBotConfig || {};
          if (!svc) {
            // Service not ready yet, queue the update
            (window as any).__hackathon_botPendingReconfigure = { lang, task };
            (window as any).logBot?.('[Reconfigure] WhisperLive service not ready; queued for later.');
            return;
          }
          cfg.language = lang;
          cfg.task = task || 'transcribe';
          (window as any).__hackathon_botBotConfig = cfg;
      ...
      try {
        const pending = (window as any).__hackathon_botPendingReconfigure;
      ...
      await new Promise<void>((resolve, reject) => {
        try {
          ...
          audioService.findMediaElements().then(async (mediaElements: HTMLMediaElement[]) => {
            ...
            (window as any).__hackathon_botOnMessage = onMessage;
            (window as any).__hackathon_botOnError = onError;
            (window as any).__hackathon_botOnClose = onClose;

            return await whisperLiveService.connectToWhisperLive(
              (window as any).__hackathon_botBotConfig,
              onMessage,
              onError,
              onClose
            );
            ...
            const initializeTeamsSpeakerDetection = (whisperLiveService: any, audioService: any, botConfigData: any) => {
              ...
              function getTeamsParticipantId(element: HTMLElement) {
                ...
                if (!id) {
                  ...
                  if (!(element as any).dataset.hackathon_botGeneratedId) {
                    (element as any).dataset.hackathon_botGeneratedId = 'teams-id-' + Math.random().toString(36).substr(2, 9);
                  }
                  id = (element as any).dataset.hackathon_botGeneratedId;
                }
              ...
              function observeTeamsParticipant(participantElement: HTMLElement) {
                ...
                if (!(participantElement as any).dataset.hackathon_botObserverAttached) {
                  (participantElement as any).dataset.hackathon_botObserverAttached = 'true';
                }
              }
              ...
              function scanForAllTeamsParticipants() {
                for (const selector of participantSelectors) {
                  ...
                  for (let i = 0; i < participantElements.length; i++) {
                    ...
                    if (!(el as any).dataset.hackathon_botObserverAttached) {
                  ...
                  if (mutation.type === 'childList') {
                    mutation.addedNodes.forEach(node => {
                      if (node.nodeType === Node.ELEMENT_NODE) {
                        ...
                        for (const selector of participantSelectors) {
                          if (elementNode.matches(selector) && !(elementNode as any).dataset.hackathon_botObserverAttached) {
                          ...
                          for (let i = 0; i < childElements.length; i++) {
                            ...
                            if (!(childEl as any).dataset.hackathon_botObserverAttached) {
                      ...
                      if (node.nodeType === Node.ELEMENT_NODE) {
                        ...
                        for (const selector of participantSelectors) {
                          if (elementNode.matches(selector)) {
                            ...
                            delete (elementNode as any).dataset.hackathon_botObserverAttached;
                            delete (elementNode as any).dataset.hackathon_botGeneratedId;
