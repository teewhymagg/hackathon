...
class DockerClient:
    ...
    def __init__(self):
        ...
        self.network_name = os.getenv("DOCKER_NETWORK", "hackathon_bot_default")
    ...
    def _count_running_bots_for_user(self, user_id: str) -> int:
        ...
        try:
            containers = self.client.containers.list(
                filters={"label": f"hackathon_bot.user_id={user_id}", "status": "running"}
            )
    ...
    def create_bot_container(self, user_id: str, meeting_id: str, meeting_url: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            container = self.client.containers.run(
                ...
                labels={"hackathon_bot.user_id": str(user_id)},
                restart_policy={"Name": "on-failure", "MaximumRetryCount": 3}
            )
            
            logger.info(f"Created container {container_name} with label hackathon_bot.user_id={user_id}")
