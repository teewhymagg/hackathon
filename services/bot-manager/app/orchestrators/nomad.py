...
# Name of the *parameterised* job that represents a hackathon_bot instance
BOT_JOB_NAME = os.getenv("HACKATHON_BOT_JOB_NAME", "hackathon_bot")
...
async def start_bot_container(
    ...
    """Dispatch a parameterised *hackathon_bot* Nomad job.
...
async def get_running_bots_status(user_id: int) -> List[Dict[str, Any]]:
    ...
    Queries the Nomad API to find all running hackathon_bot jobs and filters them
    by the user_id in the job metadata.
    """
    logger.info(f"Querying Nomad for running bots for user {user_id}")
    
    try:
        # Query Nomad for all running hackathon_bot jobs
        ...
        async with httpx.AsyncClient() as client:
            ...
            for job in jobs_data:
                # Only process hackathon_bot jobs
