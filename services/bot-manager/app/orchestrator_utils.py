...
BOT_IMAGE_NAME = os.environ.get("BOT_IMAGE_NAME", "hackathon_bot:dev")
...
async def start_bot_container(
    ...
    """
    Starts a hackathon_bot container via requests_unixsocket AFTER checking user limit.
    ...
    # These are the environment variables passed to the Node.js process  of the hackathon_bot started by your entrypoint.sh.
    environment = [
    ...
    create_payload = {
        ...
        "Env": environment,
        "Labels": {"hackathon_bot.user_id": str(user_id)}, # *** ADDED Label ***
        "HostConfig": {
...
async def get_running_bots_status(user_id: int) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        filters = json.dumps({
            "label": [f"hackathon_bot.user_id={user_id}"],
            "status": ["running"]
        })
    ...
    async with async_session_local() as db_session:
        for container_info in running_containers:
            ...
            # Parse meeting_id from name: hackathon_bot-{meeting_id}-{uuid}
            meeting_id_from_name = "unknown"
            try:
                 parts = name.split('-')
                 if len(parts) > 2 and parts[0] == 'hackathon_bot' and parts[1] == 'bot':
